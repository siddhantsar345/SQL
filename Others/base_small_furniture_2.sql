create table adhoc_ttl_90days.smallfurniture_base_19th_September_2025 as 
with fk_tpc as(
    select distinct
        account_id
    from(
...
WHERE LOWER(sales.status) IN ('in_progress', 'undelivered', 'completed', 'delivered', 'approved', 'shipped', 'ready_to_ship', 'returned', 'return_requested', 'activated')
    ...
    AND sales.is_shopsy_order = FALSE
    AND lower(sales.analytic_vertical) IN ('kitchenrack', 'clothdryerstand', 'condimentset', 'bathroomrackandshelf', 'deskorganizers', 'toothbrushholder', 'laundrybasket', 'oildispenser', 'vanitybox', 'hook', 'container', 'babypillow', 'decorwallshelf', 'mattress', 'floormat', 'garmentcover', 'walldecoration', 'kitchentoolset', 'livingroomcurtain', 'schoolwaterbottle', 'artificialplant', 'cookwareset', 'cuttingboard', 'nappy', 'tablecover', 'roleplaytoy', 'pillowcover', 'outdoortoy', 'cupandsaucer', 'keyholder')
