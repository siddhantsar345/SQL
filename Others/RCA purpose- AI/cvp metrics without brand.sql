...
FROM (
    ...
    WHERE LOWER(sales.status) IN ('in_progress', 'undelivered', 'completed', 'delivered', 'approved', 'shipped', 'ready_to_ship', 'returned', 'return_requested', 'activated')
        ...
        AND sales.marketplace_id IN ('FLIPKART')
        AND lower(sales.analytic_business_unit) IN ('bgm','home','lifestyle','furniture')
        AND sales.order_date_key BETWEEN 20260101 AND 20260421
        AND sales.is_shopsy_order = FALSE    
    GROUP BY
        sales.order_date_key,
        sales.analytic_business_unit,
...
LEFT JOIN (
    ...
    LEFT JOIN (
        ...
        WHERE order_date_key BETWEEN 20260101 AND 20260421
        GROUP BY 
            listing_id, 
    ...
    WHERE list_dim.marketplace_id = 'FLIPKART'
        ...
        AND list_dim.process_date_key BETWEEN 20260101 AND 20260421
    GROUP BY
    ...
    AND instock.branded_flag = speed.branded_flag 
    AND instock.is_alpha_seller = speed.is_alpha_seller
...
LEFT JOIN (
    ...
    FROM (
        ...
        WHERE (CAST(date_key AS int64) BETWEEN 20260101 AND 20260421)
            AND (
    ...
    AND ai_myntra.branded_flag = speed.branded_flag 
    AND ai_myntra.is_alpha_seller = speed.is_alpha_seller 
...
LEFT JOIN (
    ...
    FROM (
        ...
        INNER JOIN (
            ...
            WHERE lower(sales.status) in ('in_progress','undelivered','completed','delivered','approved','shipped','ready_to_ship','returned', 'return_requested','activated')
                ...
                AND sales.is_shopsy_order =FALSE
                AND lower(sales.analytic_business_unit) IN ('bgm','home','lifestyle','furniture')
                AND order_date_key BETWEEN 20260101 AND 20260421
            GROUP BY
                sales.listing_id,
                order_date_key
        ) fes
    ...
    AND price_drop.branded_flag = speed.branded_flag 
    AND price_drop.is_alpha_seller = speed.is_alpha_seller;