create table adhoc_ttl_90days.largefurniture_base_19th_September_2025 as 
with fk_tpc as(
    select distinct
        account_id
    from(
...
WHERE LOWER(sales.status) IN ('in_progress', 'undelivered', 'completed', 'delivered', 'approved', 'shipped', 'ready_to_ship', 'returned', 'return_requested', 'activated')
    ...
    AND sales.is_shopsy_order = FALSE
    AND lower(sales.analytic_vertical) IN ('chimney', 'stepladder', 'ceilinglamp', 'towelholder', 'waterpurifier', 'airconditioner', 'clothdryerstand', 'desktopmonitor', 'kitchentrolley', 'showerhead', 'washingmachinedryer', 'bathroommirror', 'walllamp', 'refrigerator', 'voltagestabilizer', 'dustbin', 'hometemple', 'curtainaccessories', 'vase', 'geyserwaterheater', 'aircooler', 'hosepipe', 'soapdish', 'bathroomrackandshelf', 'television', 'liquiddispenser', 'toothbrushholder', 'faucet', 'fan', 'dinnerset', 'decorwallshelf', 'gasstove', 'plantcontainer', 'laptop', 'wokandkadhai', 'laundrybasket', 'platetrayanddish', 'cookwareset', 'mousepad', 'washingmachinetrolleys', 'mensformaltrouser', 'tablebowl', 'walldecoration', 'floormat', 'livingroomcurtain', 'pillow')
