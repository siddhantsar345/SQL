SELECT
    MONTH(from_unixtime(unix_timestamp(sales.unit_creation_date_time, 'M/d/yy H:mm'))) AS month,
...
WHERE LOWER(sales.status) IN ('in_progress','undelivered','completed','delivered','approved','shipped','ready_to_ship','returned','return_requested','activated')
    ...
    AND sales.order_date_key BETWEEN 20260101 AND CAST(date_format(last_day(add_months(current_date, -1)), 'yyyyMMdd') AS BIGINT)

GROUP BY
    MONTH(from_unixtime(unix_timestamp(sales.unit_creation_date_time, 'M/d/yy H:mm'))),
