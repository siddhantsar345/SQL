create table adhoc_ttl_90days.largefurniture_base_19th_Sept_2025 as 
with fk_tpc as(
    select distinct
        account_id
    from(
...
exclude_sc as(
    ...
    WHERE LOWER(sales.status) IN ('in_progress', 'undelivered', 'completed', 'delivered', 'approved', 'shipped', 'ready_to_ship', 'returned', 'return_requested', 'activated')
        ...
        AND sales.order_date_key BETWEEN 20240918 and 20250919
        AND sales.is_shopsy_order = FALSE
        AND lower(pc.analytic_super_category) IN ('largefurniture')
)
...
WHERE LOWER(sales.status) IN ('in_progress', 'undelivered', 'completed', 'delivered', 'approved', 'shipped', 'ready_to_ship', 'returned', 'return_requested', 'activated')
    ...
    AND sales.is_shopsy_order = FALSE
    AND lower(sales.analytic_vertical) IN ('smallbedsidetables', 'washbasins', 'chimney', 'stepladder', 'microwaveoven', 'stool', 'kitchensink', 'ceilinglamp', 'airconditioner', 'waterpurifier', 'towelholder', 'vacuumcleaner', 'dustbin', 'desktopmonitor', 'clothdryerstand', 'doorlock', 'voltagestabilizer', 'kitchentrolley', 'washingmachinedryer', 'geyserwaterheater', 'exhaustfan', 'bathroommirror', 'airfryer', 'vipdigitalsubscription', 'walllamp', 'showerhead', 'refrigerator', 'doorhanger', 'cabinetdrawer', 'vase', 'drainstrainer', 'hometemple', 'hosepipe', 'curtainaccessories', 'smallbookshelf', 'tubelight', 'toiletbrush', 'printer', 'television', 'tableplacemat', 'soapdish', 'wokandkadhai', 'dinnerset', 'faucet', 'pillow', 'platetrayanddish', 'aircooler', 'liquiddispenser', 'lock', 'electriccooker', 'bathroomrackandshelf', 'garbagebags', 'tablebowl', 'laptop', 'carpetrug', 'plantcontainer', 'batteries', 'fan')
