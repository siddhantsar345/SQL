...
COUNT(DISTINCT CASE WHEN (LOWER(p.payment_instrument) IN ('phonepe', 'upi_intent', 'upi_collect')) AND sales.new_cust_flag <> 1 AND sales.new_to_bu = 1 THEN fuf.order_external_id END) AS upi_on_orders,
COUNT(DISTINCT CASE WHEN (LOWER(p.payment_instrument) IN ('phonepe', 'upi_intent', 'upi_collect')) AND sales.new_cust_flag <> 1 AND sales.new_to_bu <> 1 THEN fuf.order_external_id END) AS upi_oo_orders,
