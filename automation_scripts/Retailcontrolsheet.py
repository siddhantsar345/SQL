...
cred = '/home/siddhantsar.vc/retailserviceaccount.json'

control_gsheet_id = "11kDePI6yUBnJlh8OuwSNXhUIsQrygm1qJNPpLoz1mDk"
control_gsheet_sheet_name = "Retail"
...
# [CHANGE] Added @retry decorator to run_query.
# Previously run_query had NO retry logic — if QaaS returned a failure or the job
# stayed in 'scheduled' state, it would just fail outright with no recovery.
# Now it retries up to 5 times with exponential backoff (10s → 20s → 40s → 80s → 160s),
# giving QaaS enough time to free up capacity and move the job to 'running' state.
@retry(retries=5, delay=10, backoff=2)
def run_query(query1, source, file_path, filename, shared_token_cache, shared_auth_lock):
    # [CHANGE] Increased random sleep range from (1, 10) to (1, 20).
    # With 10 jobs firing in parallel, a max 10s stagger was not enough to prevent
    # all of them from hitting QaaS at nearly the same time.
    # Spreading them over 20s reduces simultaneous submissions and queue saturation.
    ...
    result = QaasAPI(client_id=my_details.QAAS_CLIENT_ID, rh_node_id=my_details.QAAS_NODE_ID, client_secret=my_details.QAAS_CLIENT_SECRET,
                     enable_persistence=True, max_retries=10,
...
def csv_upload_to_gdrive(ds, file_path, file_name, folder_id, overwrite, file_id):
    # [CHANGE] For overwrite jobs, the local file now has a date suffix (e.g. Speed_2026_20260712.csv)
    # to protect it from the cleanup loop. But the file name shown on Google Drive should remain
    # the original name without the suffix (e.g. Speed_2026.csv) so downstream consumers aren't affected.
    # We strip the _YYYYMMDD suffix from the Drive name for overwrite jobs only.
    drive_file_name = file_name.replace(f"_{date_till_key}.csv", ".csv") if overwrite else file_name

    file_metadata = {
        'name': drive_file_name,  # [CHANGE] Use clean name on Drive, not the date-suffixed local name
        'parents': [folder_id],
        'mimeType': 'text/csv'
    }
...
# [CHANGE] Increased parallel_workers from 3 to 5.
# With 10 jobs and only 3 workers, the last batch of jobs was waiting the longest
# before even starting — by which time tokens or QaaS sessions could expire.
# 5 workers means at most 2 batches of 5, reducing the maximum wait time significantly.
# Tune this down if QaaS has a hard concurrency limit that causes rejections.
def run_parallel(df, parallel_workers):
    with Manager() as manager:
        shared_token_cache = manager.dict()
        shared_auth_lock = manager.Lock()

        # [CHANGE] Added timeout=600 to p.map().
        # Previously, if a worker hung indefinitely (e.g. QaaS job stuck in scheduled),
        # the entire pool would block forever. Now each job gets a max of 10 minutes
        # before it's considered failed and the pool moves on.
        with ProcessPoolExecutor(max_workers=parallel_workers) as p:
            futures = [
                p.submit(process_job, row, shared_token_cache, shared_auth_lock)
            ...
            for future in futures:
                try:
                    log = future.result(timeout=600)  # [CHANGE] 10-minute per-job timeout
                    ...
                    # [CHANGE] Per-job exception handling — previously one job failure
                    # could silently affect the aggregated log. Now each is caught individually.
                    print(f"Job failed with exception: {e}")
                    aggregated_log["Query Fail"].append(f"UNKNOWN (exception: {e})")
...
if __name__ == "__main__":
    ...
    # [CHANGE] Moved control_df loading to BEFORE the cleanup block.
    # Previously cleanup ran first, but it needed control_df to know which files
    # are active — causing NameError. Now control_df is always available when cleanup runs.
    ...
    def determine_name(row):
        ...
        if row['Overwrite']:
            # [CHANGE] Previously overwrite jobs got no date suffix (e.g. "Speed_2026.csv").
            # The cleanup logic deletes any CSV where date_till_key is NOT in the filename.
            # Since overwrite filenames had no date in them, they were being deleted mid-run
            # by the cleanup loop — causing [Errno 2] No such file or directory on upload.
            # Fix: add the date suffix to overwrite filenames too so cleanup never touches them.
            # The file is still uploaded to Drive via overwrite (fileId update), so the Drive
            # file name shown on Drive is controlled separately in csv_upload_to_gdrive and
            # remains as the original name without the suffix.
            return f"{name}_{date_till_key}.csv"
    ...
    if delete_file_bool and os.path.exists(data_file_path):
        ...
        # [CHANGE] Collect all filenames this run will produce before deleting anything.
        # Previously the cleanup had no awareness of what the current run needed,
        # so overwrite files (no date suffix) could be deleted mid-run after being written
        # by the query but before being uploaded. This guard prevents that.
        active_files = set()
        for _, row in control_df.iterrows():
            _name = str(row['FileName']).strip()
            if row.get('Overwrite'):
                active_files.add(f"{_name}_{date_till_key}.csv")
            else:
                active_files.add(f"{_name}_Data_{date_till_key}.csv")

        for file in os.listdir(data_file_path):
            if file.endswith('.csv') and date_till_key not in file and file not in active_files:
    ...
    if filtered_control_df.empty:
        ...
        # [CHANGE] Pass parallel_workers as a variable rather than hardcoded,
        # consistent with the top-level config. No functional change, just cleaner.
