...
control_gsheet_table_range = "C3:L" # Update
...
def csv_upload_to_gdrive(file_path, file_name, folder_id):
    """
    Uploads a CSV to Drive. If a file with the same base name (everything
    before '_Data_') already exists in the folder, replaces its content
    and renames it to file_name. Otherwise uploads fresh.
    """
    ds = service.get_drive_service(cred, my_details.email)

    # Derive the base prefix, e.g. 'Speed_2026_Data_20260504.csv' -> 'Speed_2026_Data_'
    if '_Data_' in file_name:
        base_prefix = file_name.split('_Data_')[0] + '_Data_'
    else:
        # Fallback: strip the .csv and use the whole stem
        base_prefix = file_name.rsplit('.', 1)[0]

    safe_prefix = base_prefix.replace("'", "\\'")
    query = (
    ...
    # 'name contains' is a substring match, so filter to true prefix matches only
    existing = [f for f in existing if f['name'].startswith(base_prefix)]

    media_body = MediaIoBaseUpload(
        fd=open(file_path + file_name, 'rb'),
        mimetype='text/csv',
        resumable=True
    )
    ...
    if existing:
        ...
        if len(existing) > 1:
            print(f"Warning: {len(existing)} files matching '{base_prefix}*' found. "
                  f"Updating the first one (ID: {existing_file_id}). "
                  f"Names: {[f['name'] for f in existing]}")

        response = ds.files().update(
            fileId=existing_file_id,
            media_body=media_body,
            body={'name': file_name, 'mimeType': 'text/csv'},  # rename to new date
            supportsAllDrives=True
        ).execute()
        print(f"Replaced existing file '{existing[0]['name']}' -> '{file_name}'. File ID: {response['id']}")
        return response['id']
    else:
        file_metadata = {
            'name': file_name,
        ...
        response = ds.files().create(
            media_body=media_body,
            body=file_metadata,
            supportsAllDrives=True
        ).execute()
        print(f"Uploaded new file '{file_name}'. File ID: {response['id']}")
        return response['id']
...
def process_job(job_data, shared_token_cache, shared_auth_lock):
    ...
    try:
        if file_name in log_dict['Query Pass']:
            file_id = csv_upload_to_gdrive(data_file_path, file_name, folder_id)
...
if __name__ == "__main__":
    ...
    filtered_control_df['final_file_name'] = filtered_control_df.loc[:, 'FileName'].astype(
        str).str.strip() + '_Data_' + date_till_key + '.csv'
