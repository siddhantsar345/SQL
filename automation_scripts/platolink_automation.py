...
# Existing control sheet (same as before)
...
# NEW — Plato Links sheet (where Apps Script writes the daily URLs)
LINKS_SHEET_ID = "1ih1qKGQzX1m7b_5U9puD4QZiEHz4ll99xh6x_1tONcs"
LINKS_SHEET_NAME = "links"

# NEW — refuse to download if the URL in the sheet is older than this many hours
# (protects against silently re-uploading yesterday's data if Apps Script failed)
MAX_URL_STALENESS_HOURS = 12

# NEW — HTTP download timeout in seconds
...
# ---------------------------------------------------------------------------
# RETRY DECORATOR (unchanged from your original)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# NEW — DOWNLOAD LINK (replaces run_query)
# ---------------------------------------------------------------------------
@retry(retries=3, delay=10, backoff=2)
def download_link(identifier_key, file_path, filename):
    """
    1. Reads URL + updated_at from the Plato Links sheet for the given identifier_key
    2. Verifies URL is fresh (updated_at within MAX_URL_STALENESS_HOURS)
    3. Streams the CSV to file_path + filename
    4. Validates the downloaded file is non-empty
    ...
    try:
        ...
        # If parsing fails, log a warning but allow the download
    ...
    try:
        ...
        if status == 403:
            raise RuntimeError(
                f"HTTP 403 for '{identifier_key}': signed URL likely expired or unauthorized"
            ) from e
        if status == 404:
        ...
        if os.path.exists(partial_path):
            os.remove(partial_path)
        raise RuntimeError(
            f"Download timed out for '{identifier_key}' (>{DOWNLOAD_TIMEOUT_SECONDS}s)"
        )
    ...
    # Atomic rename — only swap to the real filename after successful download
...
# ---------------------------------------------------------------------------
# UPLOAD / DELETE / EMAIL (unchanged from your original)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# PROCESS JOB (modified — query step replaced with download step)
# ---------------------------------------------------------------------------
def process_job(job_data, shared_token_cache, shared_auth_lock):
    """
    Handles one row from the control sheet:
      1. Reads link from Plato Links sheet (download_link)
      2. Uploads CSV to Drive (csv_upload_to_gdrive)
      3. Sends success/failure email
    """
    # CHANGED: IdentifierKey replaces Query+Source. We reuse the Query column in
    # the control sheet to hold the identifier_key (e.g. 'keyword_level_search_count').
    ...
    # ---- Download Link (replaces Run Query) ----
    try:
...
# ---------------------------------------------------------------------------
# PARALLEL RUNNER (unchanged, only log keys renamed)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# MAIN (unchanged from your original)
# ---------------------------------------------------------------------------
