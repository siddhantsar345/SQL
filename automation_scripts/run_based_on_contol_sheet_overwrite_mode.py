...
overwrite_mode = False # Update
file_id = None # Update, provide file_id if overwrite_mode is True and you want to overwrite a specific file, else it will create a new file with same name and upload  
...
def csv_upload_to_gdrive(ds, file_path, file_name, folder_id, overwrite, file_id):
    ...
    with open(file_path + file_name, 'rb') as f:
        ...
        if not overwrite:
            ...
            response = ds.files().update(fileId=file_id, body=file_metadata, media_body=media_body).execute()
...
if __name__ == "__main__":
    ...
    control_df['Overwrite'] = control_df['File_Id'].apply(lambda x: True if x else False)
