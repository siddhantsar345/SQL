...
case when fk_price between 0 and 500 then '0-500'
...
when fk_price>15000 then '15k+' else "" end as price_bucket,
       b.branded_flag as branded_flag,
b.brand_tier as brand_tier,
        fsn_landscape,
        SUM(search_ppvs) AS search_ppvs,
        ...
        SUM(
            CASE 
                WHEN fsn_landscape = 'FK_Comp' THEN (1 * search_ppvs) 
                WHEN fsn_landscape = 'AI_Comp' THEN (-1 * search_ppvs) 
                ELSE 0 
            END
        ) AS ci_cal
...
and lower(comp.analytic_vertical) = lower(b.analytic_vertical)
   ...
   comp.date_key ,
        ...
        case when lower(comp.fk_seller_type) = 'fk-alpha' then 'Diamond'
              when lower(comp.fk_seller_type) = 'fk-non_alpha' then 'Rest of MP' else 'Others' end ,
        case when lower(comp.az_seller_type)='az-alpha' then 'Diamond'
        when lower(comp.az_seller_type)='az-non_alpha' then 'Rest of MP' else 'Others' end ,
...
when fk_price between 7501 and 10000 then '7.5k-10k'
when fk_price between 10001 and 15000 then '10k-15k'
when fk_price>15000 then '15k+' else "" end,
       b.branded_flag ,
b.brand_tier ,
        fsn_landscape
