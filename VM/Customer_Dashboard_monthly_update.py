...
start_day=date.today()-timedelta(days=1)       ## d-1 date
end_day=date.today()-timedelta(days=2)         ## d-2 date use for ty

last_30_day=date.today()-timedelta(days=31)


start_day_ty='{:02d}'.format(start_day.year)+'{:02d}'.format(start_day.month)+'01'
end_day_ty='{:02d}'.format(end_day.year)+'{:02d}'.format(end_day.month)+'{:02d}'.format(end_day.day)
end_day_live_ty='{:02d}'.format(start_day.year)+'{:02d}'.format(start_day.month)+'{:02d}'.format(start_day.day)

start_day_ly='{:02d}'.format(start_day.year-1)+'{:02d}'.format(start_day.month)+'01'
end_day_ly='{:02d}'.format(start_day.year-1)+'{:02d}'.format(start_day.month)+'{:02d}'.format(start_day.day) ## d-1 date ly

end_day_lydm2='{:02d}'.format(end_day.year-1)+'{:02d}'.format(end_day.month)+'{:02d}'.format(end_day.day) ## d-2 Date ly



last30_ty='{:02d}'.format(last_30_day.year)+'{:02d}'.format(last_30_day.month)+'{:02d}'.format(last_30_day.day)
last30_ly='{:02d}'.format(last_30_day.year-1)+'{:02d}'.format(last_30_day.month)+'{:02d}'.format(last_30_day.day)
# -
...
user_name='deepakkumar.e.vc' ### Change as per your username
...
( 
	...
	where 
		...
		and a.order_date_key  between 20251001 and 20251031
...
(	
	...
	where
	sa.approve_date_key between 20251001 and 20251031
	group by
	sa.account_id,
	sa.analytic_business_unit
) base
...
( 
	...
	(
		...
		where
	 	...
	 	and c.new_to_bu=1
	 	and c.approve_date_key between 20251001 and 20251031
		group by
	...
	(
		...
		and ca.new_to_bu=0
		and ca.approve_date_key between 20251001 and 20251031
		group by
...
where
sales_fact.order_date_key between 20251001 and 20251031
...
gc=pygsheets.authorize(service_file='ShivankAutomation-fd5dab44a774.json')
sh=gc.open('Customer Dashboard MOM Raw')
...
data.set_dataframe(cust_data,(1,1),copy_head=True,copy_index=False,extend=True)
...
X_Client_Id= 'analytics-cdo' ### Change as per your org-namespace
X_Client_Secret= '6cbdc3cd-a3f7-43e1-a2f9-1935f6317c44' ### Change as per your org-namespace
user_name='rishika.manojpatwa' ### Change as per your username
...
left join bigfoot_external_neo.scp_ekl__logistics_geo_hive_dim loc
on sales_fact.pincode = loc.pincode

left join bigfoot_external_neo.cp_uie__Affluence_V3_2_final_output_fact afff
...
data.set_dataframe(cust_data,(1,38),copy_head=True,copy_index=False,extend=True)
...
X_Client_Id= 'analytics-cdo' ### Change as per your org-namespace
X_Client_Secret= '6cbdc3cd-a3f7-43e1-a2f9-1935f6317c44' ### Change as per your org-namespace
user_name='ashrut.garg' ### Change as per your username

# Alpha Query you want to pull
query = """

------ raw_data_m1_cuts ----- (J:O) ------
...
(
    ...
    where
        ...
        and order_date_key between 20251001 and 20251031 ------ Change to Current Month
...
(
    ...
    from
        bigfoot_external_neo.analytics_cdo__customer_insight_hive_fact
    where 
        approve_date_key < 20251001    ----- Start of the month
    group by 
        account_id,
        analytic_business_unit
 ) b
...
data.set_dataframe(repeat,(1,10),copy_head=True,copy_index=False,extend=True)
...
---- TPC for current month for Customer dashboard (BU level MOM) --------
---- raw_tpc --- (A:G) ------
...
data.set_dataframe(tpc,(1,1),copy_head=True,copy_index=False,extend=True)





# # For BU x SC level (TPC)
...
---- TPC for current month for Customer dashboard (BU x SC level MOM) --------
...
sheet_name = [s.title for s in worksheet]
data=sh[sheet_name.index('raw_tpc')]
data.clear(start='L2', end='S1000', fields="*")
data.set_dataframe(sc_tpc,(1,12),copy_head=True,copy_index=False,extend=True)
...
----------- BU x SC level Geo tag level Customers MOM ---------
...
data.set_dataframe(cust_data_1,(1,1),copy_head=True,copy_index=False,extend=True)
...
-------- BU x SC level M1,M2,M3 Cust (MOM) -----------
...
(
    ...
    where
        ...
        and category_id !=21726
        and category_id !=21651
        and order_date_key between 20251001 and 20251031
        and lower(marketplace_id) = 'flipkart'
        and (replacement_for_unit is null or replacement_for_unit='not_replacement')
        and (exchange_for_unit is null or exchange_for_unit='not_exchange')
        and is_freebie =false
...
on 
    ...
    and a.analytic_business_unit = b.analytic_business_unit 
    and a.analytic_super_category = b.analytic_super_category
group by
    substr(a.order_date_key,1,6),
...
data.set_dataframe(repeat,(1,1),copy_head=True,copy_index=False,extend=True)
...
# +
# myntra_dump = pd.read_csv("C:\\Users\deepakkumar.e.vc\Downloads\core_inv_check.csv")
