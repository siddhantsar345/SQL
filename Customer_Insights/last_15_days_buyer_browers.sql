...
WHERE
    ...
    AND t2.analytic_vertical IN ('MensRaincoatUnbranded', 'ElectronicHobbyKits', 'CameraLensProtector', 'ScrewDriverSet', 'HomeAudioSpeaker', 'CyclingLight', 'DigitalComboVAS', 'MenFashionChain', 'KeyChain', 'CnFHighTops', 'HomeSecurityCamera', 'WaistBag', 'GymGlove', 'Soundbar', 'PowerBank', 'SmartWatchScreenGuard', 'Torch', 'CnFBoots', 'MenSunglass', 'MobileProtectionMobileScreenGuard', 'CnFMisc', 'HandheldAirFreshner')
    AND t1.accountid NOT IN (
        ...
        WHERE
            ...
            AND lower(sales.analytic_vertical) IN (
                'mensraincoatunbranded', 'electronichobbykits', 'cameralensprotector', 'screwdriverset', 'homeaudiospeaker', 'cyclinglight', 'digitalcombovas', 'menfashionchain', 'keychain', 'cnfhigh tops', 'homesecuritycamera', 'waistbag', 'gymglove', 'soundbar', 'powerbank', 'smartwatchscreenguard', 'torch', 'cnfboots', 'mensunglass', 'mobileprotectionmobilescreenguard', 'cnfmisc', 'handheldairfreshner')
    )