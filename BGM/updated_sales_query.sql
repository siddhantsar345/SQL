...
(SELECT 
    ...
    SUM(CASE WHEN sales.gmv / sales.units <= 150 THEN sales.units END) AS 0_150_units,
    SUM(CASE WHEN sales.gmv / sales.units > 150 AND sales.gmv / sales.units <= 300 THEN sales.units END) AS 150_300_units,
...
LEFT JOIN 
    ...
    WHERE UPPER(return_type) = 'CUSTOMER_RETURN' 
            AND (
             (order_item_approve_date_key BETWEEN 20240101 AND CAST(CONCAT('2024', date_format(date_sub(current_date, 1), 'MMdd')) AS INT))
            OR (order_item_approve_date_key BETWEEN 20250101 AND CAST(date_format(date_sub(current_date, 1), 'yyyyMMdd') AS INT)) 
            )
...
LEFT JOIN 
    ...
    WHERE sales.type IN ('physical')
            ...
            AND (
             (sales.order_date_key BETWEEN 20240101 AND CAST(CONCAT('2024', date_format(date_sub(current_date, 1), 'MMdd')) AS INT))
            OR (sales.order_date_key BETWEEN 20250101 AND CAST(date_format(date_sub(current_date, 1), 'yyyyMMdd') AS INT)) 
            )
...
LEFT JOIN 
        ...
        FROM bigfoot_external_neo.analytics_cdo__customer_insight_hive_fact
          ...
          WHERE  (
          (approve_date_key BETWEEN 20240101 AND CAST(CONCAT('2024', date_format(date_sub(current_date, 1), 'MMdd')) AS INT))
          OR (approve_date_key BETWEEN 20250101 AND CAST(date_format(date_sub(current_date, 1), 'yyyyMMdd') AS INT))
          )
...
LEFT JOIN
  (
    ...
    WHERE
       (
       (ff.fulfill_item_unit_order_date_key BETWEEN 20240101 AND CAST(CONCAT('2024', date_format(date_sub(current_date, 1), 'MMdd')) AS INT))
       OR (ff.fulfill_item_unit_order_date_key BETWEEN 20250101 AND CAST(date_format(date_sub(current_date, 1), 'yyyyMMdd') AS INT))
       )
...
LEFT JOIN
  (
    select 
        ...
        (sales.order_date_key BETWEEN 20240101 AND CAST(CONCAT('2024', date_format(date_sub(current_date, 1), 'MMdd')) AS INT))
        OR
        (sales.order_date_key BETWEEN 20250101 AND CAST(date_format(date_sub(current_date, 1), 'yyyyMMdd') AS INT))
        )
        and sales.order_date_key not in (20240229)
        AND sales.is_shopsy_order = FALSE
