...
COUNT(DISTINCT (CASE WHEN lower(p.payment_instrument) = 'phonepe' AND (sales.new_cust_flag <> 1 AND sales.new_to_bu = 1 AND sales.new_to_sc = 1)  THEN fuf.order_external_id END)) as phonepe_ONN_Orders, 
COUNT(DISTINCT (CASE WHEN lower(p.payment_instrument) = 'phonepe' AND (sales.new_cust_flag <> 1 AND sales.new_to_bu <> 1 AND sales.new_to_sc = 1)  THEN fuf.order_external_id END)) as phonepe_OON_Orders, 
COUNT(DISTINCT (CASE WHEN lower(p.payment_instrument) = 'phonepe' AND (sales.new_cust_flag <> 1 AND sales.new_to_bu <> 1 AND sales.new_to_sc <> 1)  THEN fuf.order_external_id END)) as phonepe_OOO_Orders,
SUM((CASE WHEN lower(p.payment_instrument) = 'phonepe' THEN fuf.gmv END)) AS upi_GMV,
SUM((CASE WHEN lower(p.payment_instrument) = 'phonepe' THEN fuf.units END)) AS upi_Units,
SUM((CASE WHEN lower(p.payment_instrument) = 'phonepe' THEN fuf.gmv END))/COUNT(DISTINCT(CASE WHEN lower(p.payment_instrument) = 'phonepe' OR lower(p.payment_instrument) = 'upi_intent' OR lower(p.payment_instrument) = 'upi_collect' THEN fuf.order_external_id END)) as upi_AOV,
...
left join bigfoot_external_neo.cp_bi_prod_sales__forward_payments_365_fact p
...
AND fuf.order_date_key between 20240501 and 20240531
