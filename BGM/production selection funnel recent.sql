...
new_selection_ppv AS (
    ...
    FROM (
        ...
        LEFT JOIN bigfoot_external_neo.sp_product__product_attribute_hive_dim prod_dim ON sales.product_id = prod_dim.product_id
        LEFT JOIN (SELECT listing_id, MAX(flipkart_selling_price) as flipkart_selling_price, CAST(MIN(listing_created_on) AS DATE) as listing_birth_date FROM bigfoot_external_neo.sp_product__listing_hive_dim WHERE marketplace_id = 'FLIPKART' AND listing_created_on >= '2024-07-01' GROUP BY 1) lhd ON sales.listing_id = lhd.listing_id
...
discovery_metrics AS (
    ...
    FROM (
        SELECT
            sales.listing_id, sales.product_id, sales.analytic_business_unit, sales.analytic_super_category, sales.analytic_vertical, sales.service_profile,
