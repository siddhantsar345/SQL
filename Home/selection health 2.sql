...
FROM (
    SELECT 
        ...
        CASE 
            WHEN LOWER(base.final_service_profile) IN ('fbf', 'fbf_and_fbf_lite', 'fbf_and_non_fbf', 'fbf_lite') THEN 
                CASE 
                    WHEN base.total_ppvs >= 1000 THEN 'Selection with >= 1000 PPVs'
                    WHEN base.total_ppvs >= 50   THEN 'Selection with >= 50 PPVs'
                    WHEN base.total_ppvs >= 10   THEN 'Selection with >= 10 PPVs'
                    WHEN base.total_ppvs >= 2    THEN 'Selection with 2-9 PPVs'
                    WHEN base.total_ppvs = 1    THEN 'Selection with 1 PPV'
                    ELSE 'No PPVs'
                END
    ...
    FROM (
        SELECT 
            ...
            sales_agg.service_profile AS final_service_profile,
        ...
        LEFT JOIN (
            SELECT 
                listing_id, 
                MAX(service_profile) AS service_profile,
