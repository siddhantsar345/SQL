...
LEFT JOIN 
    ...
    WHERE UPPER(return_type) = 'CUSTOMER_RETURN' 
            AND (
             (order_item_approve_date_key BETWEEN 20240501 AND CAST(CONCAT('2024', date_format(date_sub(current_date, 1), 'MMdd')) AS INT))
            OR (order_item_approve_date_key BETWEEN 20250501 AND CAST(date_format(date_sub(current_date, 1), 'yyyyMMdd') AS INT)) 
            )
...
LEFT JOIN 
    ...
    WHERE sales.type IN ('physical')
            ...
            AND (
             (sales.order_date_key BETWEEN 20240501 AND CAST(CONCAT('2024', date_format(date_sub(current_date, 1), 'MMdd')) AS INT))
            OR (sales.order_date_key BETWEEN 20250501 AND CAST(date_format(date_sub(current_date, 1), 'yyyyMMdd') AS INT)) 
            )
...
LEFT JOIN 
        ...
        FROM bigfoot_external_neo.analytics_cdo__customer_insight_hive_fact
          ...
          WHERE  (
          (approve_date_key BETWEEN 20240501 AND CAST(CONCAT('2024', date_format(date_sub(current_date, 1), 'MMdd')) AS INT))
          OR (approve_date_key BETWEEN 20250501 AND CAST(date_format(date_sub(current_date, 1), 'yyyyMMdd') AS INT))
          )
...
LEFT JOIN
  (
    ...
    WHERE
       (
       (ff.fulfill_item_unit_order_date_key BETWEEN 20240501 AND CAST(CONCAT('2024', date_format(date_sub(current_date, 1), 'MMdd')) AS INT))
       OR (ff.fulfill_item_unit_order_date_key BETWEEN 20250501 AND CAST(date_format(date_sub(current_date, 1), 'yyyyMMdd') AS INT))
       )
...
LEFT JOIN
  (
    select 
        ...
        (sales.order_date_key BETWEEN 20240501 AND CAST(CONCAT('2024', date_format(date_sub(current_date, 1), 'MMdd')) AS INT))
        OR
        (sales.order_date_key BETWEEN 20250501 AND CAST(date_format(date_sub(current_date, 1), 'yyyyMMdd') AS INT))
        )
        and sales.order_date_key not in (20240229)
        AND sales.is_shopsy_order = FALSE
