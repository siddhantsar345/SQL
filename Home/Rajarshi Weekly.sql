...
( 
    ...
    WHERE lower(competitor) = 'ai'
        AND lower(ci_business_unit) in ('home','furniture')
        AND CAST(date_key AS BIGINT) >= 20260101
...
WHERE lower(comp.analytic_business_unit) in ('home','furniture')
    AND CAST(date_key AS BIGINT) between 20260101 and 20260329
...
where lhd.marketplace_id = 'FLIPKART' 
    ...
    and CAST(listing_created_on AS DATE) between '2026-01-01' and '2026-03-29'

 group by 1,2,3,4,5,6,7



----- Selection with 1 ppv -- run the queery
...
(
    ...
    LEFT JOIN (
        ...
        FROM (
            ...
            WHERE tag_end_date <= 20260419
        ) as a
        WHERE rn = 1
    ) hipo ON sales.product_id = hipo.product_id
    ...
    WHERE sales.marketplace_id='FLIPKART' 
        and sales.date_key between 20260401 and 20260419
        and lower(sales.analytic_business_unit) in ('home')
...
LEFT JOIN  fdp_uploads.ds_fkint_cp_santa_hyperlocal_vertical_bu_mapping_2_0 AS hl 
    ...
    AND LOWER(hl.bu_final) in ('home','furniture')
...
WITH weekly_snapshot AS (
    ...
    WHERE list_dim.process_date_key between 20260401 and 20260419
...
) b ON b.product_id = list_dim.product_id
...
where lhd.marketplace_id = 'FLIPKART' 
    AND lower(lhd.analytic_business_unit) in ('home')
    and CAST(listing_created_on AS DATE) >= '2026-01-01'

 group by 1,2,3,4,5,6;


--------- % Average Price increase & % DW with price increase v/s baseline (>1%)
...
SELECT
   ...
   case when output_weighted_cur_asp/output_weighted_bau_asp > 1.01 then 1 else 0 end as price_inc_flag,
...
GROUP BY
   ...
   calc.analytic_business_unit ,
   calc.analytic_super_category ,
  case when output_weighted_cur_asp/output_weighted_bau_asp > 1.01 then 1 else 0 end ,
   calc.is_alpha_seller