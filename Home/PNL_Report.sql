...
--CN_formula
case when cnxfsn_formula = "Gross Units" then cnxfsn_value * units
...
case when overall_cogs_im_percentage is null or  overall_cogs_im_percentage = 0 then ((units - (overall_rto_units_percentage * units)-(overall_rvp_units_percentage * units)) * cogsxfsn__cost_per_unit )
...
when overall_cogs_im_percentage > 0 and  cogsxfsn__cost_per_unit > 0 then


GREATEST(((units - (overall_rto_units_percentage * units)-(overall_rvp_units_percentage * units)) * cogsxfsn__cost_per_unit ), ((((mrp / units) * (units * (1-overall_rto_units_percentage-overall_rvp_units_percentage))) * (1-overall_cogs_im_percentage))/(1+overall_tax_percentage)))
else 0 end as final_cogs_im ,
...
CASE
   ...
   THEN
       (CASE
           WHEN adsxfsn__absolute_or_percentage = 'GMV %'
               THEN adsxfsn__value * gmv
           WHEN adsxfsn__absolute_or_percentage = 'Net MRP %'
               THEN adsxfsn__value * ((mrp / units) * (units * (1-overall_rto_units_percentage-overall_rvp_units_percentage)))
           WHEN adsxfsn__absolute_or_percentage = 'Net Units'
               THEN adsxfsn__value * (units - (units * overall_rto_units_percentage) - (units * overall_rvp_units_percentage))
           WHEN adsxfsn__absolute_or_percentage = 'Gross MRP %'
               THEN adsxfsn__value * mrp
           WHEN adsxfsn__absolute_or_percentage = 'NMV %'
               THEN adsxfsn__value * (gmv - (gmv * overall_rto_gmv_percentage) - (gmv * overall_rvp_gmv_percentage))
           WHEN adsxfsn__fsn IS NOT NULL AND adsxfsn__absolute_or_percentage IS NULL AND adsxfsn__value > 1
               THEN adsxfsn__value * (units - (units * overall_rto_units_percentage) - (units * overall_rvp_units_percentage))
           WHEN adsxfsn__fsn IS NOT NULL AND adsxfsn__absolute_or_percentage IS NULL AND adsxfsn__value < 1
               THEN adsxfsn__value * (gmv - (gmv * overall_rto_gmv_percentage) - (gmv * overall_rvp_gmv_percentage)) -- Corrected this line
           ELSE NULL -- Optional: Handles cases that don't match any of the above
       END)
...
WHEN LOWER(analytic_super_category) NOT IN ('autoaccessorys', 'toysandss') and adsxfsn1__fsn is null
   THEN
       (CASE
           WHEN adsxfsn1__absolute_or_percentage = 'GMV %'
               THEN adsxfsn1__value * gmv
           WHEN adsxfsn1__absolute_or_percentage = 'Net MRP %'
               THEN adsxfsn1__value * ((mrp / units) * (units * (1-overall_rto_units_percentage-overall_rvp_units_percentage)))
           WHEN adsxfsn1__absolute_or_percentage = 'Net Units'
               THEN adsxfsn1__value * (units - (units * overall_rto_units_percentage) - (units * overall_rvp_units_percentage))
           WHEN adsxfsn1__absolute_or_percentage = 'Gross MRP %'
               THEN adsxfsn1__value * mrp
           WHEN adsxfsn1__absolute_or_percentage = 'NMV %'
               THEN adsxfsn1__value * (gmv - (gmv * overall_rto_gmv_percentage) - (gmv * overall_rvp_gmv_percentage))
           WHEN adsxfsn1__fsn IS NOT NULL AND adsxfsn1__absolute_or_percentage IS NULL AND adsxfsn1__value > 1
               THEN adsxfsn1__value * (units - (units * overall_rto_units_percentage) - (units * overall_rvp_units_percentage))
           WHEN adsxfsn1__fsn IS NOT NULL AND adsxfsn1__absolute_or_percentage IS NULL AND adsxfsn1__value < 1
               THEN adsxfsn1__value * (gmv - (gmv * overall_rto_gmv_percentage) - (gmv * overall_rvp_gmv_percentage)) -- Corrected this line
           ELSE NULL -- Optional: Handles cases that don't match any of the above
       END)
...
THEN
   ...
   CASE
       ...
       WHEN sum((gmv - (gmv * overall_rto_gmv_percentage) - (gmv * overall_rvp_gmv_percentage))) over (partition by analytic_super_category, brand) = 0
       THEN 0
       ELSE
      
           ((gmv - (gmv * overall_rto_gmv_percentage) - (gmv * overall_rvp_gmv_percentage)))
           /
           (sum((gmv - (gmv * overall_rto_gmv_percentage) - (gmv * overall_rvp_gmv_percentage))) over (partition by analytic_super_category, brand))
           * adsxtoyssxbrand__total_ads_amount
           * adsxtoyssxdayxper__day_wise_ads_percentage
   END
   -- Third condition for the 'autoaccessorys' category
   WHEN LOWER(analytic_super_category) IN ('autoaccessorys')
   THEN
       0.1019 * (gmv - (gmv * overall_rto_gmv_percentage) - (gmv * overall_rvp_gmV_percentage))
  
   ELSE NULL -- Optional: Handles any other categories not covered
END AS ads
...
on
lower(cnxfsn.analytic_super_category)=lower(sales1.analytic_super_category) and
cnxfsn.fsn=sales1.product_id and
--lower(cnxfsn.brand)=lower(sales1.brand) and
cnxfsn.date = sales1.order_date_key


--COGSXFSN
left join
fdp_uploads.ds_fkint_analytics_cdo_pnl_inputs_fsn_x_month_level_cogs_2_0 as cogsxfsn
on
lower(cogsxfsn.analytic_super_category)=lower(sales1.analytic_super_category) and
cogsxfsn.fsn=sales1.product_id and
cast(cogsxfsn.yearmo as string) = substring(cast(sales1.order_date_key as string),0,6)
...
on
cast(imxfsn.yearmo as string)=substring(cast(sales1.order_date_key as string),0,6) and
lower(imxfsn.analytic_super_category)=lower(sales1.analytic_super_category) and
imxfsn.fsn= sales1.product_id
...
on
cast(imxsc.yearmo as string)=substring(cast(sales1.order_date_key as string),0,6) and
lower(imxsc.analytic_super_category) = lower(sales1.analytic_super_category)
...
on
rto_rvp_fsn.fsn=sales1.product_id and
lower(rto_rvp_fsn.analytic_super_category)=lower( sales1.analytic_super_category) and
cast(rto_rvp_fsn.yearmo as string)= substring(cast(sales1.order_date_key as string),0,6) and
(rto_rvp_fsn.rto_percentage_units > 0 OR rto_rvp_fsn.rvp_percentage_units > 0) and
(rto_rvp_fsn.rto_percentage_gmv > 0 or rto_rvp_fsn.rvp_percentage_value > 0 )
...
on
cast(rto_rvp_scxverticalbrand.yearmo as string)=substring(cast(sales1.order_date_key as string),0,6) and
lower(rto_rvp_scxverticalbrand.analytic_super_category) = lower(sales1.analytic_super_category) and
lower(rto_rvp_scxverticalbrand.analytic_verticalxbrand) =lower(CONCAT(sales1.analytic_vertical,sales1.brand)) and
(rto_rvp_scxverticalbrand.rto_percentage_units > 0 or rto_rvp_scxverticalbrand.rvp_percentage_units > 0) and
(rto_rvp_scxverticalbrand.rto_percentage_gmv > 0 or  rto_rvp_scxverticalbrand.rvp_percentage_value > 0 )
...
on
cast(rto_rvp_scxvertical.yearmo as string)=substring(cast(sales1.order_date_key as string),0,6) and
lower(rto_rvp_scxvertical.analytic_super_category)=lower(sales1.analytic_super_category) and
lower(rto_rvp_scxvertical.analytic_vertical)=lower(sales1.analytic_vertical) and
(rto_rvp_scxvertical.rto_percentage_units > 0 or  rto_rvp_scxvertical.rvp_percentage_units > 0) and
(rto_rvp_scxvertical.rto_percentage_value > 0 or  rto_rvp_scxvertical.rvp_percentage_value > 0)


--RTO_RVP_SC
left join fdp_uploads.ds_fkint_analytics_cdo_pnl_inputs_sc_x_month_level_rto_rvp_1_0 as rto_rvp_sc
on
cast(rto_rvp_sc.yearmo as string)= substring(cast(sales1.order_date_key as string),0,6) and
lower(rto_rvp_sc.analytic_super_category)=lower(sales1.analytic_super_category) and
(rto_rvp_sc.rto_percentage_units > 0 or rto_rvp_sc.rvp_percentage_units > 0) and
(rto_rvp_sc.rto_percentage_value > 0  or rto_rvp_sc.rvp_percentage_value > 0)


--Tax_SC X Vertical
left join  fdp_uploads.ds_fkint_analytics_cdo_pnl_inputs_sc_x_month_level_tax_1_0 as tax_scxvertical
on
cast(tax_scxvertical.yearmo as string) = substring(cast(sales1.order_date_key as string),0,6) and
lower(tax_scxvertical.analytic_super_category) = lower(sales1.analytic_super_category) and
lower(tax_scxvertical.analytic_vertical) = lower(sales1.analytic_vertical)
...
on
cast(tax_fsn.yearmo as string)=substring(cast(sales1.order_date_key as string),0,6) and
tax_fsn.fsn = sales1.product_id
...
on
cast(blxsc.yearmo as string)=substring(cast(sales1.order_date_key as string),0,6) and
lower(blxsc.analytic_super_category) = lower(sales1.analytic_super_category)


--SCmxscxvertical
left join fdp_uploads.ds_fkint_analytics_cdo_pnl_inputs_sc_x_vertical_level_month_scm_2_0 as scmxscxvertical
on
cast(scmxscxvertical.yearmo as string)=substring(cast(sales1.order_date_key as string),0,6) and
lower(scmxscxvertical.analytic_super_category) = lower(sales1.analytic_super_category) and
lower(scmxscxvertical.analytic_vertical) = lower(sales1.analytic_vertical)
...
on
cast(scmxscxfsn.yearmo as string)=substring(cast(sales1.order_date_key as string),0,6) and
lower(scmxscxfsn.analytic_super_category) = lower(sales1.analytic_super_category) and
scmxscxfsn.fsn = sales1.product_id


--SCmxsc
left join fdp_uploads.ds_fkint_analytics_cdo_pnl_inputs_sc_level_month_scm_1_0 as scmxsc
on
cast(scmxsc.yearmo as string)=substring(cast(sales1.order_date_key as string),0,6) and
lower(scmxsc.analytic_super_category) = lower(sales1.analytic_super_category )
...
on
adsxfsn.date = sales1.order_date_key and
lower(adsxfsn.analytic_super_category)=lower(sales1.analytic_super_category) and
adsxfsn.fsn = sales1.product_id and
--lower(adsxfsn.brand) = lower(sales1.brand) and
adsxfsn.fsn is not null


left join fdp_uploads.ds_fkint_analytics_cdo_pnl_inputs_sc_x_fsn_level_month_ads_1_0 as adsxfsn1
on
adsxfsn1.date = sales1.order_date_key and
lower(adsxfsn1.analytic_super_category)=lower(sales1.analytic_super_category) and
lower(adsxfsn1.brand) = lower(sales1.brand) and
adsxfsn1.fsn is null
...
on
--total_ads_amount
cast(adsxtoyssxbrand.yearmo as string)=substring(cast(sales1.order_date_key as string),0,6) and
lower(adsxtoyssxbrand.analytic_super_category)=lower(sales1.analytic_super_category) and
lower(adsxtoyssxbrand.brand) = lower(sales1.brand )
...
on
adsxtoyssxdayxper.date = sales1.order_date_key
