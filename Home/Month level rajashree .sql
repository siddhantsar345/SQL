SELECT
    CAST(SUBSTR(CAST(sales.order_date_key AS STRING), 1, 6) AS INT) AS year_month,
...
WHERE
    ...
    AND sales.order_date_key between 20260101 and 20260228
    AND LOWER(sales.analytic_business_unit) = 'home'

GROUP BY
CAST(SUBSTR(CAST(sales.order_date_key AS STRING), 1, 6) AS INT),
sales.analytic_business_unit