        ...
        FROM
            (
                SELECT
                    ...
                    CAST(DATE_FORMAT('%Y%m%d', approved_date_time) as double) as order_date_key,
                ...
                WHERE
                    ...
                    AND CAST(DATE_FORMAT('%Y%m%d', approved_date_time) as double) between 20250801 and 20250831
                    AND order_date_key >= 20250101
                    AND sales.is_alpha_seller = TRUE
                    AND sales.is_shopsy_order = FALSE
                GROUP BY
                    prod_dim.analytic_business_unit,
                    CAST(DATE_FORMAT('%Y%m%d', approved_date_time) as double),
                    sales.product_id,
