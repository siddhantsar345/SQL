...
base AS (
 ...
 LEFT JOIN fdp_uploads.ds_fkint_cp_santa_hyperlocal_vertical_bu_mapping_2_0 AS hl
   ...
   WHERE
    ...
    AND sales.order_date_key BETWEEN 20251201 AND 20260701
...
order_events AS (
 SELECT
     ...
     e.event_date,
     e.campaign,
     RANK() OVER (PARTITION BY o.account_id, o.order_date ORDER BY e.event_time DESC) AS rn
 FROM base o
...
GROUP BY analytic_super_category, event_date, segment
...
SELECT
 ...
 SUM(gmv) AS gmv
FROM segmented
GROUP BY campaign, event_date, segment
