    SELECT
        t3.product_id,
        t3.total_july_buyers,
        t3.total_repurchasers,
        (CAST(t3.total_repurchasers AS DOUBLE) / t3.total_july_buyers) * 100 AS repurchase_rate_percentage
    FROM
        (
            SELECT
                t1.product_id,
                COUNT(DISTINCT t1.account_id) AS total_july_buyers,
                COUNT(DISTINCT t2.account_id) AS total_repurchasers
            FROM
                (
                    SELECT DISTINCT
                        sales.account_id,
    ...
    WHERE
        t3.total_july_buyers >= 100
    ORDER BY
        repurchase_rate_percentage DESC,
        t3.total_july_buyers DESC